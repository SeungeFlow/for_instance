...
## HISTORY_DIRECTORY_RELOCATION_20260620_V1 — history directory 전환

사건(event):
root `history_00.md`의 기존 history 본문을 `history/history_20260620.md`로 보존하고,
root `history_00.md`는 gpt.history 기준장 진입점 stub으로 전환했다.

날짜(date):
2026-06-20

주의(date caution):
20260620은 relocation/capture 기준일이다.
기존 모든 사건의 실제 발생일로 단정하지 않는다.

기준 입력(source):
```yaml
instance: gpt.history
source_review: gpt.history_HISTORY_DIRECTORY_RELOCATION_REVIEW_20260620.md
verdict: ACCEPT_WITH_REFINEMENT
target_change:
  from: history_00.md
  to: history/history_20260620.md
root_stub_needed: YES
preserve_existing_history: YES
final_guard: "relation is not merge"
```
